...
```bash
ROOT="$(git rev-parse --show-toplevel)" && pi --no-extensions --no-themes --no-context-files --no-session --theme "$ROOT/themes/oscura-midnight.json" --extension "$ROOT/extensions/oscura-theme.ts"
```
...
Inside Pi, run `/oscura` and verify:
...
If `/oscura` is missing, first confirm that the command resolved this repository:

```bash
ROOT="$(git rev-parse --show-toplevel)" && test -f "$ROOT/extensions/oscura-theme.ts" && printf 'extension found: %s\n' "$ROOT/extensions/oscura-theme.ts"
```
