...
```bash
ROOT="$(git rev-parse --show-toplevel)" && pi --no-extensions --no-themes --no-context-files --no-session --theme "$ROOT/themes/oscura-midnight.json" --extension "$ROOT/extensions/pi-grok-build-ui.ts"
```
...
Inside Pi, run `/pi-grok-build-ui` and verify:
...
If `/pi-grok-build-ui` is missing, first confirm that the command resolved this repository:

```bash
ROOT="$(git rev-parse --show-toplevel)" && test -f "$ROOT/extensions/pi-grok-build-ui.ts" && printf 'extension found: %s\n' "$ROOT/extensions/pi-grok-build-ui.ts"
```
